...
Provides a user-friendly CLI for managing projects and tasks with PostgreSQL persistence.
"""
...
class TodoCLI:
    ...
    def create_project(self) -> None:
        ...
        name = input("Enter project name (max 30 words): ").strip()
        if not name:
            print("Project name cannot be empty.")
            return

        description = input("Enter project description (max 150 words): ").strip()
        if not description:
    ...
    def list_projects(self) -> None:
        ...
        for i, project in enumerate(projects, 1):
            task_count = len(self.storage.get_project_tasks(project.id))
            print(f"{i}. [{project.id}]")
            print(f"   Name: {project.name}")
            print(
                f"   Description: {project.description[:100]}{'...' if len(project.description) > 100 else ''}"
            )
            print(f"   Tasks: {task_count}")
            print(f"   Created: {project.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
            print()

    def _pick_project(self, action: str):
        projects = self.storage.list_projects()
        if not projects:
        ...
        try:
            project_choice = int(input(f"Select project to {action} (number): ")) - 1
        ...
        return str(projects[project_choice].id)

    def update_project(self) -> None:
        print("\nUpdate Project")
        print("-" * 30)

        project_id = self._pick_project("update")
        if not project_id:
            return
        ...
        print(
            f"Current description: {project.description[:100]}{'...' if len(project.description) > 100 else ''}"
        )
    ...
    def delete_project(self) -> None:
        ...
        project_id = self._pick_project("delete")
        if not project_id:
            return
    ...
    def create_task(self) -> None:
        ...
        project_id = self._pick_project("use")
        if not project_id:
            return

        title = input("Enter task title (max 30 words): ").strip()
        if not title:
            print("Task title cannot be empty.")
            return

        description = input("Enter task description (max 150 words): ").strip()
        if not description:
    ...
    def list_project_tasks(self) -> None:
        ...
        project_id = self._pick_project("view")
        if not project_id:
            return
        project = self.storage.get_project(project_id)
        if not project:
    ...
    def update_task(self) -> None:
        ...
        project = self.storage.get_project(project_id)
        if not tasks:
            print(f"No tasks found in project '{project.name if project else project_id}'.")
            return

        print(f"\nTasks in '{project.name}':")
        for i, task in enumerate(tasks, 1):
            print(f"{i}. [{task.id}] {task.title} (Status: {task.status.value})")
        ...
        print(
            f"Current description: {task.description[:100]}{'...' if len(task.description) > 100 else ''}"
        )
        if task.deadline:
    ...
    def update_task_status(self) -> None:
        ...
        try:
            self.storage.update_task(task_id=task.id, status=new_status)
